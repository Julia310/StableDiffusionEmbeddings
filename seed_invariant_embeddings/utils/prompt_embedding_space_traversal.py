...
def construct_image(image_list):
    ...
    # Create a blank image with combined dimensions
    combined_image = Image.new('RGB', (combined_width, combined_height))

    # Paste each image into the combined image at the right position
...
def display_embedding_path_traversal(values):
    ...
    # Display the image
    fig, ax = plt.subplots()
    ax.imshow(combined_image, extent=[-0.01, 1., -8.5, 8.5])  # Using extent to define axis bounds
    plt.autoscale(enable=False)

    # Plot the provided values as a solid line
