...
class GradientDescent(torch.nn.Module):
    ...
    def get_text_embedding(self, interpolation = False):
        ...
        if interpolation:
            condition = ldm.slerp(self.condition, self.target_condition, torch.sigmoid(self.pre_alpha))
            #condition = get_shifted_embedding(condition, self.default_std, self.default_mean)
    ...
    def forward(self, i, region_indices, metric, interpolation = False):
        ...
        latents = ldm.embedding_2_img('', self.get_text_embedding(interpolation), save_img=False, dim=dim, return_pil=False,
...
if __name__ == '__main__':
    with torch.no_grad():
        target_latents = ldm.embedding_2_img('', ldm.get_embedding([prompt])[0], save_img=False, dim=dim, seed=target_seed, return_pil=False,
                                             return_latents=True, keep_init_latents=False)

        target_init_latents = torch.clone(ldm.initial_latents)
        ldm.embedding_2_img('', ldm.get_embedding([prompt])[0], dim=dim, seed=seed, return_latents=True, keep_init_latents=False)
        ...
        for region in ['complete', 'every 4', 'every 4 alternating', 'center']:
            for mod in [2, 10]:
                for learning_rates in [(10, 0.001), (1, 0.001) ,(0.01, 0.001), (0.1, 0.001), (0.01, 0.0001), (0.1, 0.0001), (1, 0.0001), (10, 0.0001)]:
                    ...
                    gd_condition = GradientDescent(ldm.text_enc([prompt]), ldm.text_enc([prompt2]), target_latents, combined_init_latents)
                    gd_condition.alpha = torch.nn.Parameter(gd_condition.alpha)
                    gd_condition.alpha.requires_grad = True
                    gd_init_latents = GradientDescent(ldm.text_enc([prompt]), ldm.text_enc([prompt2]), target_latents, combined_init_latents)
                    ...
                    for i in range(mod * 35):
                        ...
                        if (i+1) % mod != 0: #i >= 0:
                            ...
                            score = gd_condition.forward(int((i-cnt + 1)/2), region, score_metric, interpolation=True)
                            print(int((i-cnt + 1)/2))
                            ...
                            pil_img.save(f'output/interpolation/{dir_num}/{i}_B_{prompt[0:25]}_{round(score.item(), 3)}.jpg')
                    ...
                    plot_scores(init_latents_dist_list, f'output/interpolation/{dir_num}/init_latent_distances.jpg', x_label='Iterations', y_label=score_metric)
                    plot_scores(scores_list, f'output/interpolation/{dir_num}/similarity_scores.jpg', x_label='Iterations', y_label=score_metric)
                    plt.clf()
                    plot_scores(interpolation_value, f'output/interpolation/{dir_num}/interpolation_values.jpg', x_label='Iterations',
                                y_label='alpha')
                    plt.clf()
