...
def get_embedding():
    image_path = r'D:\StableDiffusionEmbeddings\output\adamOnLion\prompt1\384_a beautiful painting of a peaceful lake in th_7.016696929931641.jpg'
    image = Image.open(image_path)
    _, preprocess = clip.load("ViT-L/14", device='cuda')
    image = preprocess(image).unsqueeze(0).half().to('cuda')
    c = ldm.image_encoder(image)
    print(c)
