...
class GradientDescent(torch.nn.Module):
    ...
    def forward(self, i, region_indices, metric, seed_batch, loss = 'latents'):
        print(seed_batch)
        score = 0
        for j in range(len(seed_batch)):
            ...
            if loss == 'latents':
                ...
                latents = ldm.embedding_2_img('', self.get_text_embedding(), return_latents_step=69, dim=dim,
                                              seed=seed_batch[j], return_pil=False)

                image = ldm.latents_to_image(latents, return_pil=False)

                score = compute_dist_metric(metric, self.target_image, image) + score
...
if __name__ == '__main__':
    ...
    for eta in [0.1]:
        os.mkdir(f'./output/seed_ind_gen/7_1/{eta}')
        for score_metric in ['Cosine Similarity', 'Euclidean Distance']:
            for region in ['complete']:
                for mod in [2]:
                    val = 0.01

                    dir_num = create_next_directory(f'output/seed_ind_gen/7_1/{eta}')
...
#
                            pil_img.save(
                                f'output/seed_ind_gen/7_1/{eta}/{dir_num}/417016_{i}_{prompt[0:25]}_{round(score.item(), 3)}_{round(val, 2)}.jpg')
                            del pil_img
